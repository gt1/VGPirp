...
**/
#if ! defined(BRPHEADER_HPP)
#define BRPHEADER_HPP

#include <map>
#include "VGP_FileType.hpp"
#include "VGP_Provenance.hpp"
#include "VGP_BRPReaderException.hpp"
	...
	void readHeaderLines(LineBuffer & LB)
	{
		...
		while ( LB.getline(&la,&le) )
		{
			...
			if ( la != le && !isalpha(*la) )
			{
				...
				if ( la == le || *la != ' ' )
				{
					throw BRPReaderException(std::string("BRPReader::readHeaderLines: no space after plus in ") + std::string(linestart,le));
				}
				la++;
				...
				else
				{
					if ( la == le )
					{
						throw BRPReaderException(std::string("BRPReader::readHeaderLines: no line type in ") + std::string(linestart,le));
					}
					...
					if ( la == le || *la != ' ' )
					{
						throw BRPReaderException(std::string("BRPReader::readHeaderLines: no space after line type type in ") + std::string(linestart,le));
					}
...
std::ostream & operator<<(std::ostream & out, BRPHeader const & H);
#endif
